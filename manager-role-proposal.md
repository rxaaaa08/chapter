...
| **6. Scorecard** — ✅ **BUILT 2026-07-18** (`20260718_manager_role_phase6_scorecard.sql` applied to prod; verified vs live benchmark 21.7%) | `get_manager_scorecards()` (strict-admin) → chips on the Managers card: fill %, conversion vs all-events avg, stale, pending-age, recovery %, doubt closure %, revenue, actions/7d, hires, last seen. Plus `weekly_manager_digest()` on pg_cron (Mon 6:15pm IST): one info alert per manager in the Briefing tab (own `manager_scorecard` rule row — additive, the 36KB `evaluate_manager_rules()` was deliberately not touched) + one combined push; disable it from the Briefing rules list | Low, read-only |
