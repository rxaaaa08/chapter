-- Founders-only gate on get_performance_summary(). The RPC returns the whole
-- P&L (monthly profit, forecast, per-event margins, marketer ROI) and was
-- granted to `authenticated` with no internal role check — any staff login
-- could call it via the REST API. That mattered little while the Performance
-- tab was admin-only UI; now that managers get a (money-free) Performance
-- tab, the data itself must refuse non-founders: the final SELECT is wrapped
-- in CASE WHEN is_admin_strict() so everyone else gets NULL. Body otherwise
-- identical to 20260718_manager_role_phase5_money.sql (the live def).
