-- Manager role — Phase 5: money.
-- 1) events.manager_commission — optional per-event ₹/ticket override
--    (NULL = the manager's own default, ₹35).
-- 2) accrue_manager_sale() — when a lead flips to fully_paid, the event's
--    manager (earliest-assigned active one, if any) earns a manager_sales
--    row. Amount snapshotted like marketer sales; idempotent via the
--    UNIQUE(application_id) + ON CONFLICT DO NOTHING.
-- 3) get_performance_summary() — manager commission now subtracts from the
--    committed-profit model (drives "Made this month" + the 6-month
--    forecast + committed_total) and is exposed per event as
--    manager_commission_per_ticket for the unit-economics table. Subtracted
--    ONLY when the event actually has an active manager assigned.
--    NOTE: the live definition was dumped from prod before editing (it is
--    ahead of the repo's committed migrations) — this file contains the
--    full current definition plus the manager additions.

ALTER TABLE public.events ADD COLUMN IF NOT EXISTS manager_commission numeric(10,2);
...
BEGIN
  IF NEW.status = 'fully_paid' AND OLD.status IS DISTINCT FROM 'fully_paid' THEN
    SELECT m.id, COALESCE(e.manager_commission, m.commission_amount)
      INTO v_mgr, v_amount
    FROM event_managers em
    JOIN managers m ON m.id = em.manager_id AND m.active
    JOIN events   e ON e.slug = em.event_slug
    WHERE em.event_slug = NEW.event_slug
    ORDER BY em.created_at
    LIMIT 1;
    IF v_mgr IS NOT NULL THEN
      INSERT INTO manager_sales (application_id, manager_id, amount)
      VALUES (NEW.id, v_mgr, v_amount)
...
DROP TRIGGER IF EXISTS trg_accrue_manager_sale ON public.applications;
CREATE TRIGGER trg_accrue_manager_sale
  AFTER UPDATE OF status ON public.applications
  FOR EACH ROW EXECUTE FUNCTION public.accrue_manager_sale();

-- ── Performance summary with manager cost ─────────────────────────────────
...
-- Active manager per event (earliest-assigned wins — the accrual trigger
-- pays the same one, so forecast and ledger agree).
ev_manager AS (
...
ev_agg AS (
  ...
  GROUP BY e.id, e.title, e.slug, e.cost_per_ticket, e.is_active, e.marketer_commission,
           e.manager_commission, mgr.commission_amount
