-- Net the founder's committed-profit numbers of AFFILIATE (creator) commissions,
-- the same way marketer commission is already subtracted. Only affects events
-- with affiliate_enabled = true that have an attributed application — so it's a
-- no-op until creator links start converting. Commission = affiliate_commission_pct%
-- of the city-aware configured full price, matching accrue_affiliate_sale().
--
-- Only ev_committed changes vs 20260621_performance_summary_single_payment_aware;
