-- Founders-only gate on get_analytics_summary(). Same exposure class as
-- get_performance_summary: SECURITY DEFINER, granted to `authenticated`,
-- no internal role check — any staff login could pull business-wide
-- analytics (visitors, funnels, per-event conversion) via the REST API even
-- though the Analytics tab is admin-only UI. Non-founders now get NULL.
-- get_experiments_daily was checked too and is ALREADY gated live.
--
-- Body reproduced from the LIVE definition (dumped 2026-07-19 — the live fn
-- is ahead of the repo's committed migrations, incl. pricing_by_city), with
-- only the final SELECT wrapped in CASE WHEN is_admin_strict().
