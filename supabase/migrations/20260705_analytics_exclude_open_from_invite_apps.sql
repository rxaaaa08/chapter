-- Fix: open events were polluting the invite "Payment Conversion" number.
--
-- get_analytics_summary.apps_per_event computes, per event, `approved`
-- (status invited/advance_paid/fully_paid) and `advance_paid` (advance_paid/
-- fully_paid). The Analytics tab reads these as the invite funnel's
-- "approved → paid" conversion. But OPEN events (booking_url='payu-hosted')
-- have no approval step — an open booking jumps straight from pending to
-- advance_paid/fully_paid, so every paid open booking counts as BOTH approved
-- AND paid, i.e. a fake 100% conversion. Once open events get volume they
-- inflate the pooled invite Payment Conversion rate.
--
-- Fix: restrict apps_per_event to non-open events. `IS DISTINCT FROM` (not
-- `<>`) so events with a NULL booking_url are kept, exactly as before — the
-- ONLY behaviour change is that open events drop out. Open events get their
-- own funnel (details → pay → paid, with abandonment + recovery) in a
-- separate block; nothing else in this function changes.
