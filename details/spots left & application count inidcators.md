...
## ⚠️ UPDATE — 2026-06-24: calendar spots-left is now PER-DATE

The calendar "spots left" indicator was reworked from **per-event** to **per-date**.
The sections further down describe the *old* per-event behavior — read this block
first; where it conflicts, **this block wins.** (We now host only native
application events, so per-date is the single source of truth.)

### Corrections to stale notes below
- **`fetchEventCounts()` no longer runs direct `applications` queries.** After the
  RLS lockdown, anon can't read `applications`. It now calls the SECURITY DEFINER
  RPC **`event_booking_counts`** (resolves `invite_slug` → canonical slug). The
  `registered`/`reserved` semantics are unchanged.
- **The ApplicationForm "🔥 Only X spots left" line (old § 3) no longer exists** in
  the code. It was removed in an earlier change; there is nothing to maintain there.

### New data source (per-date)
- New SECURITY DEFINER RPC **`event_booking_counts_by_date(p_slug)`** →
  `{ selected_date, registered, reserved }[]`, grouped by `applications.selected_date`.
  Anon-safe, resolves `invite_slug` → canonical slug.
  Migration: `supabase/migrations/20260624_event_booking_counts_by_date_rpc.sql`.
- **`fetchEventDateCounts(slug)`** in `src/supabase.ts` returns a map
  `{ 'YYYY-MM-DD': { registered, reserved } }`.
- `src/AppFlow.tsx` holds it in `dateCounts` state, fetched alongside the existing
  counts when a native event is selected, and passes it to `EventDetailsOverlay`.
- `applications.selected_date` is stored as the **base event date** (the calendar
  undoes `cityDateOffset` before submit), so `dateCounts` keys line up 1:1 with
  `event.dates[].date`.

### Per-date model (capacity applies to EACH date)
For each trip date `d` (capacity = `event.totalCapacity`, same number per date):
- `reserved_d` = advance_paid/fully_paid applicants who chose `d`
- `available_d = capacity − reserved_d`
- `available_d ≤ 0` → **auto sold-out** (grey "SOLD/OUT" cell — computed, not a DB
  `sold_out` status; native event dates are forced to `available` in the DB).
- else `reserved_d / capacity ≥ 0.5` → that date is **filling fast**.
- else → **available**.

**Earliest filling-fast date** = earliest *by date* among filling-fast dates that
still have spots. It is the ONLY date shown amber, and it drives "Only X left"
(X = its `available_d`). Every other non-sold date renders **green**.

### Legend rules (top of the calendar)
- ≥1 filling-fast date → `Only X spots left` (earliest FF date) **+** `Available`
  *(the Available key only if at least one date is green)*.
- No filling-fast date (but ≥1 green) → `Filling fast` + `Available` (default key).
- All dates sold out → **no keys** (the grey cells communicate it).

### Booking-timeline social proof is now per-date
`{application_count}` → `(3 × capacity) + registered-for-the-SELECTED-date`
(was `+ event-wide applicationCount`). Hidden until `dateCounts` loads.

### Code pointers (current)
```txt
src/supabase.ts                  fetchEventDateCounts(), fetchEventCounts() (RPC)
src/AppFlow.tsx  dateCounts state + fetch (the counts useEffect)
src/AppFlow.tsx  EventDetailsOverlay.renderCalendar() — per-date legend/cells/sold-out
src/AppFlow.tsx  Booking-timeline prize row — per-date socialProofCount
supabase/migrations/20260624_event_booking_counts_by_date_rpc.sql
```
