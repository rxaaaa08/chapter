...
import { readRawBody, verifySignature, logStatus, logInbound } from './_wamafy.js';
...
export default async function handler(req, res) {
  ...
  try {
    if (event === 'message.status') {
      ...
      await logInbound(d, payload);
    } else {
      // Unrecognised topic. Keep the payload, but pass no messageId -- otherwise
      // logStatus would create a stub row in whatsapp_sends for something that
      // was never a send.
      await logStatus({
        messageId: null,
    ...
    // permanently, and an event subscription auto-disables after 15 consecutive
    // failures -- taking every future message with it.
